...
# you need to.
is_rule1 = IF('male (?x)', THEN('is (?x) (?x)'))
is_rule2 = IF('female (?x)', THEN('is (?x) (?x)'))
sibling_rule = IF(AND('parent (?p) (?c1)', 'parent (?p) (?c2)', NOT('is (?c1) (?c2)')), THEN('sibling (?c1) (?c2)'))
cousin_rule = IF(AND('parent (?p1) (?x)', 'parent (?p2) (?y)', "sibling (?p1) (?p2)"), THEN('cousin (?x) (?y)'))
brother_rule = IF(AND('sibling (?k1) (?k2)', 'male (?k1)'), THEN('brother (?k1) (?k2)'))
sister_rule = IF(AND('sibling (?k1) (?k2)', 'female (?k1)'), THEN('sister (?k1) (?k2)'))
father_rule = IF(AND('parent (?p) (?k)', 'male (?p)'), THEN('father (?p) (?k)'))
mother_rule = IF(AND('parent (?p) (?k)', 'female (?p)'), THEN('mother (?p) (?k)'))
son_rule = IF(AND('parent (?p) (?k)', 'male (?k)'), THEN('son (?k) (?p)'))
daughter_rule = IF(AND('parent (?p) (?k)', 'female (?k)'), THEN('daughter (?k) (?p)'))
grandparent_rule = IF(AND('parent (?gp) (?p)', 'parent (?p) (?gc)'), THEN('grandparent (?gp) (?gc)'))
grandchild_rule = IF(AND('parent (?gp) (?p)', 'parent (?p) (?gc)'), THEN('grandchild (?gc) (?gp)'))
# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [is_rule1, is_rule2, sibling_rule, cousin_rule, brother_rule, sister_rule, father_rule, mother_rule,
                son_rule, daughter_rule, grandparent_rule, grandchild_rule]
...
WHAT_I_FOUND_INTERESTING = 'How this related to other classes/things I\'ve learned (at my internship, what I\'ve heard about logic class, etc.'
